defmodule Example16.Accounts.RBAC do
  @moduledoc """
  Documentation for `Rbac`.
  Source: https://github.com/dwyl/rbac
  """
  require Logger
  ...
  from a JSON file and caches the list in-memory (ETS)
  for fast access.
  """
  def init_roles_cache(path) do
    {:ok, roles} =
      File.read!(path)
  ...
  def get_role_from_cache(term) do
    case :ets.lookup(:roles_cache, term) do
      ...
      [] ->
        Logger.error(
          "rbac.ex:108 Role not found in ets: #{term} \n#{Exception.format_stacktrace()}"
        )
  ...
  @doc """
  Returns a list of the roles found in the cache.
  """
  def valid_roles(roles) do
    List.wrap(roles)
    |> Enum.map(&get_role_from_cache(&1))
    |> Enum.filter(fn role -> Map.has_key?(role, :name) && role.id != 0 end)
    |> Enum.uniq()
  end
